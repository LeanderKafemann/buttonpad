...
def main() -> None:
    ...
    # State
    board: List[List[int]] = [[0 for _ in range(ROWS)] for _ in range(COLS)]  # board[c][r]
    current: int = 1  # 1 = red, 2 = black
    game_over: bool = False

    # Cache elements
    buttons: List[BPButton] = [pad[c, 0] for c in range(COLS)]  # type: ignore[assignment]
    cells: List[List[BPLabel]] = [[pad[c, r + 1] for r in range(ROWS)] for c in range(COLS)]  # type: ignore[list-item]
